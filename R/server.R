mlemurServer <- function(input, output, session) {
  ...
  observeEvent(input$calculate, {
    ...
    if (userData$errors == TRUE) {
      ...
      if (userData$setPerPlate == 1) outputData <- calc.rate.int(userData)[1:15] else outputData <- calc.rate.per.plate.int(userData)[1:15]
      results <- data.frame("Calculations" = outputData,
                            row.names = c("&epsilon;", "N<sub>t</sub>", "CV", "&rho;", "&lambda;", "d", ifelse(userData$setPerPlate == 1, "m<sub>p</sub>", "&mu;<sub>p</sub>"), "&phi;",
                                          "m", "m<sup>CI&percnt;&ndash;</sup>", "m<sup>CI&percnt;&plus;</sup>", "&mu;", "&mu;<sup>CI&percnt;&ndash;</sup>", "&mu;<sup>CI&percnt;&plus;</sup>", "CI&percnt;"),
                            check.names = FALSE)
  ...
  rv$Ratereport <- data.frame(
    "Parameters" = c("Plating eff", "Nt", "CV", "Fitness", "Lag", "Death rate", "Residual m", "Rel size of inoc", "m", "m_low", "m_up", "mu", "mu_low", "mu_up", "CI_level")
  )
  ...
  observeEvent(input$sample, {
    ...
    callModule(settingsPlatingUpdate, "SettingsRate")
  })
  ...
  observeEvent(input$calculate2, {
    ...
    if (userDataStrain1$errors == TRUE || userDataStrain2$errors == TRUE) {
      ...
      if (userDataStrain1$setPerPlate == 1) Strain1 <- calc.rate.int(userDataStrain1)[1:16] else Strain1 <- calc.rate.per.plate.int(userDataStrain1)[1:16]
      if (userDataStrain2$setPerPlate == 1) Strain2 <- calc.rate.int(userDataStrain2)[1:16] else Strain2 <- calc.rate.per.plate.int(userDataStrain2)[1:16]
      results2 <- data.frame("Strain 1" = Strain1[1:15],
      ...
      if (userDataStrain1$setPerPlate == 1) statistics <- calc.pval.int.new(PlatingList = list(userDataStrain1, userDataStrain2), M = c(as.numeric(Strain1[16]), as.numeric(Strain2[16])), combo_rate = FALSE) else statistics <- calc.pval.per.plate.int.new(list(userDataStrain1, userDataStrain2), combo_rate = FALSE)
      pvalue <- statistics[1]
  ...
  observeEvent(input$sample2, {
    ...
    callModule(settingsPlatingUpdate, "SettingsPval")
  })
  ...
  for (i in 1:rv$FoldMaxStrainNum) {
    rv[[paste0("CountsStrain", i, "FoldUserInput")]] <- callModule(countsPlating, id = paste0("CountsStrain", i, "Fold"), userSettings=reactive(rv$SettingsFoldUserInput()), stack_cols=TRUE)
    loadDatasetWrapper(paste0("CountsStrain", i, "Fold"), rv = rv, SettingsPlatingID = "SettingsFold")
  }
  ...
  observe({
    ...
    rv$FoldEquationState <- tryCatch(eval(parse(text=paste("(function(", paste("X", 1:rv$FoldMaxStrainNum, collapse = ", ", sep = ""), "){", rv$FoldEquationText, "})(", paste((1:20)[1:rv$FoldMaxStrainNum], collapse = ", "), ")", sep = ""))), error = function(e) NA)
    ...
    rv$FoldUsedDatasets <- sapply(paste("X", 1:rv$FoldMaxStrainNum, sep = ""), function(x) {length(grep(x, rv$FoldEquationVector))})
  ...
  observe({
    ...
    if (rv$FoldNumberOfStrains == rv$FoldMaxStrainNum){
      for (i in 3:rv$FoldMaxStrainNum){
      ...
      for (i in (rv$FoldNumberOfStrains+1):rv$FoldMaxStrainNum){
        eval(parse(text = paste("shinyjs::hide('FoldColumn", i, "')", sep = "")))
      }
    }
  })
  ...
  observeEvent(input$calculate5, {
    userData <- vector("list", rv$FoldMaxStrainNum)
    names(userData) <- paste0("Strain", 1:rv$FoldMaxStrainNum)
    for (i in 1:rv$FoldMaxStrainNum) {
      userData[[paste0("Strain", i)]] <- rv[[paste0("CountsStrain", i, "FoldUserInput")]]()
    }
    
    if (any(c(sapply(paste0("Strain", 1:rv$FoldMaxStrainNum), function(x) {userData[[x]]$errors})[1:rv$FoldNumberOfStrains], !is.null(need(rv$FoldEquationState, message = F))))) {
      ...
      foldExpr <- gsub(pattern = paste("([1-", rv$FoldMaxStrainNum, "])", sep = ""), replacement = "\\[\\1\\]", x = foldExpr)
      ...
      outputData <- unlist(calc.fold.int(userData, rv$FoldEquationText))
      
      results5 <- data.frame(
        "Calculations" = outputData,
        row.names = c("f", "f<sup>CI&percnt;&ndash;</sup>", "f<sup>CI&percnt;&plus;</sup>",
                      "CI&percnt;", paste0("&mu;<sub>", 1:rv$FoldMaxStrainNum, "</sub>"))[1:length(outputData)],
        check.names = FALSE
      )
  ...
  observeEvent(input$erase5, {
    for (item in paste0("CountsStrain", 1:rv$FoldMaxStrainNum, "Fold")[1:input$FoldNumberOfStrains]) {
  ...
  observeEvent(input$sample5, {
    for (item in paste0("CountsStrain", 1:rv$FoldMaxStrainNum, "Fold")[1:input$FoldNumberOfStrains]) {
      callModule(countsPlatingUpdate, item, usePreset = (1+((which(paste0("CountsStrain", 1:rv$FoldMaxStrainNum, "Fold") == item)+1)%%2)))
    }
    callModule(settingsPlatingUpdate, "SettingsFold")
  })
          ...
          tagList(
            ...
            {tagList(div(class = "checkboxdiv",
                     ...
                     conditionalPanel(condition = "input.BatchCalculatePvalues",
                                      shinyWidgets::awesomeRadio("batchPvalueMethod",
                                                                 label = HTML(paste("Select what to compare:", infoTooltip("All together - checks whether the rates for all datasets are equal (similar to one-way ANOVA). Each to each - makes pairwise comparisons of all possible combinations of datasets. First to each - compares  the first dataset to every other dataset."))),
                                                                 choices = c("All together" = "anova", "Each to each" = "each", "First to each" = "first"),
                                                                 selected = "anova",
                                                                 width = "100%"),
                                      conditionalPanel(condition = "input.batchPvalueMethod != 'anova'",
  ...
  observeEvent(input$calculate4, {
    ...
    rv$RateOutput <- data.frame("Parameters" = c("eff", "Nt", "CV", "Fitness", "Lag", "Death", "Residual", "Inoculum", "m", "m_low", "m_up", "mu", "mu_low", "mu_up", "CI_level"), DataRate, check.names = FALSE)

    if (input$BatchCalculatePvalues) {
      
      if (input$batchPvalueMethod == "anova") {
        proxy <- calc.pval.int.new(PlatingList = rv$PlatingListCopy, M = rep(NA, length(rv$PlatingListCopy)), combo_rate = TRUE)
        DataPvalue <- matrix(proxy,nrow=1,ncol=2,dimnames=list(c("Value"), c("P value", "Comb. rate")))
        
        output$BatchPvalueResults <- reactable::renderReactable({
          DataPvalueR <- DataPvalue
          DataPvalueR[2] <- colourPvalue(DataPvalue[2])
          DataPvalueR[1] <- colourPvalue(DataPvalue[1])
          rownames(DataPvalueR) <- "<b>Value</b>"
          defaultTable(DataPvalueR, "Parameter")
        })
        
        rv$PvalueOutput <- data.frame("Parameter" = rownames(DataPvalue), as.data.frame(DataPvalue, check.names = FALSE))
        
      } else if (input$batchPvalueMethod == "first") {
        table.length <- length(DataRate)
        xarg <- 1
        yarg <- 2:length(DataRate)
        
        proxy <- sapply(yarg, function(x) {calc.pval.int.new(PlatingList = list(rv$PlatingListCopy[[1]], rv$PlatingListCopy[[x]]), M = c(NoOfMutations[[1]], NoOfMutations[[x]]), combo_rate = FALSE)})
        DataPvalue <- matrix(proxy,nrow=xarg,ncol=length(yarg),dimnames = list(names(DataRate)[xarg],names(DataRate)[yarg]))
        ...
        output$BatchPvalueResults <- reactable::renderReactable({
          DataPvalueR <- sapply(yarg-1, function(x) {colourPvalue(DataPvalue[x])})
          DataPvalueR <- matrix(DataPvalueR,nrow=xarg,ncol=length(yarg))
          colnames(DataPvalueR) <- names(DataRate)[yarg]
          rownames(DataPvalueR) <- paste("<b>", names(DataRate)[xarg], "</b>", sep = "")
          defaultTable(DataPvalueR, "Strain")
        })
        
        rv$PvalueOutput <- data.frame("Strain" = rownames(DataPvalue), as.data.frame(DataPvalue, check.names = FALSE))
        ...
        proxy <- mapply(function(x,y) {calc.pval.int.new(PlatingList = list(rv$PlatingListCopy[[x]], rv$PlatingListCopy[[y]]), M = c(NoOfMutations[[x]], NoOfMutations[[y]]), combo_rate = FALSE)}, x = xarg, y = yarg)
        ...
        DataPvalue[upper.tri(DataPvalue)] <- t(DataPvalue)[upper.tri(DataPvalue)]
