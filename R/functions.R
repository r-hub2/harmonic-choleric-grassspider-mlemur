...
#' @param data a vector of integer-valued colony counts.
#' @param e plating efficiency; a positive number not bigger than 1.
#' @param w mutant relative fitness; a positive number. Cannot be used together with lag.
#' @param lag phenotypic lag; a non-negative number. Cannot be used together with w.
#' @param poisson average number of residual Poisson-distributed mutations on the plate; a non-negative number.
#' @param death death probability of wild-type and mutant cells; a non-negative number smaller than 0.5; relative (d)eath rate
#' and death (p)robability are connected by the relation d = p/(1-p); p = d/(1+d).
#' @param phi relative size of the inoculum (N0/Nt); a non-negative number.
...
mle.mutations <- function(data, e=1, w=1, lag=0, poisson=0, death=0, phi=0, cv=0, confint=TRUE, ci.level=0.95, verbose=FALSE) {
  if (w!=1 && lag!=0) {if (verbose) message("Phenotypic lag cannot be used together with mutant fitness."); if (confint==TRUE) {return(c(NA,NA,NA))} else {return(c(NA))}}
  n <- max(data)+1
  if (cv > 0) k <- 1/cv/cv else k <- 0
  seq <- tryCatch(aux.seq(e = e, w = w, death = death, lag = lag, phi = phi, n = n-1), error = function(err) {if(verbose) message(paste("Error in aux.seq:", err)); return(NA)})
...
#' simplified and optimised to avoid redundant computations. It uses a hybrid Newton-bisection
#' algorithm as well as arbitrary-precision arithmetic if necessary for better stability.
#' @param data a list of length 2 or longer containing numeric vectors with colony counts.
#' @param e a list or vector of length 2 or longer (same as data) containing respective values of plating efficiency. Assumed 1 if left empty.
#' @param w a list or vector of length 2 or longer (same as data) containing respective values of relative mutant fitness. Assumed 1 if left empty.
#' @param lag a list or vector of length 2 or longer (same as data) containing respective values of phenotypic lag. Assumed 0 if left empty.
#' @param poisson a list or vector of length 2 or longer (same as data) containing respective values of the number of residual mutations. Assumed 0 if left empty.
#' @param death a list or vector of length 2 or longer (same as data) containing respective values of relative death rates. Assumed 0 if left empty.
#' @param phi a list or vector of length 2 or longer (same as data) containing respective values of inoculum sizes as a fraction of total culture sizes. Assumed 0 if left empty.
#' @param cv a list or vector of length 2 or longer (same as data) containing respective values of coefficient of variation. Assumed 0 if left empty.
#' @param Nt a list or vector of length 2 or longer (same as data) containing respective values of average number of cells in culture. Assumed 1 if left empty.
#' @param M a list or vector of length 2 or longer (same as data) containing MLEs of mutation rates for given datasets (if known beforehand). Mostly for internal use.
#' @param combo_rate specifies if the combined mutation rate should be returned.
#' @param verbose if TRUE, mlemur will print messages to the console.
#' @return a single value of p-value between 0 and 1, or a vector of length 2 containing p-value and combined mutation rate.
#' @export
#' @examples
#' lrt.mutations(data = list(c(26, 9, 16, 34, 15, 25, 77, 13, 14, 19),
#' c(67, 12, 112, 24, 2151, 159, 102, 60, 32, 26)), e = c(0.5, 1))
#' @examples
#' lrt.mutations(data = list(c(37, 1, 5, 43, 53, 11, 82, 2, 19, 58, 28, 70, 9, 14, 5,
#' 9, 25, 55, 2, 41, 19), c(37, 1, 5, 43, 53, 11, 82, 2, 19, 58, 28,
#' 70, 9, 14, 5, 9, 25, 55, 2, 41, 19)), lag = c(1, 2))
#' @references
#' Zheng Q. Statistical and algorithmic methods for fluctuation analysis with SALVADOR as an implementation. Math Biosci.
#' 2002;176: 237–252. doi:10.1016/S0025-5564(02)00087-1
#' @references
...
#' Zheng Q. rSalvador: An R package for the fluctuation experiment. G3 Genes, Genomes, Genet.
#' 2017;7: 3849–3856. doi:10.1534/g3.117.300120
lrt.mutations <- function(data, e=NA, w=NA, lag=NA, poisson=NA, death=NA, phi=NA, cv=NA, Nt=NA, M=NA, combo_rate=FALSE, verbose=FALSE) {
  
  # Checking data length
  if (!is.list(data) | !length(data) > 1) {if (verbose) message("Colony counts must be a list of length minimum 2."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  datasetsnum <- length(data)
  
  if (any(is.null(e)) || any(is.na(e))) {e <- rep(1, datasetsnum)}
  e <- unlist(e)
  if (length(e) < datasetsnum) {if (verbose) message("The vector of plating efficiencies is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(e) > datasetsnum) {if (verbose) message(paste("The vector of plating efficiencies is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); e <- e[1:datasetsnum]}
  
  if (any(is.null(cv)) || any(is.na(cv))) {cv <- rep(0, datasetsnum)}
  cv <- unlist(cv)
  if (length(cv) < datasetsnum) {if (verbose) message("The vector of coefficients of variation is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(cv) > datasetsnum) {if (verbose) message(paste("The vector of coefficients of variation is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); cv <- cv[1:datasetsnum]}
  
  if (any(is.null(w)) || any(is.na(w))) {w <- rep(1, datasetsnum)}
  w <- unlist(w)
  if (length(w) < datasetsnum) {if (verbose) message("The vector of mutant relative growth rates is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(w) > datasetsnum) {if (verbose) message(paste("The vector of relative growth rates is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); w <- w[1:datasetsnum]}
  
  if (any(is.null(lag)) || any(is.na(lag))) {lag <- rep(0, datasetsnum)}
  lag <- unlist(lag)
  if (length(lag) < datasetsnum) {if (verbose) message("The vector of values of phenotypic lag is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(lag) > datasetsnum) {if (verbose) message(paste("The vector of values of phenotypic lag is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); lag <- lag[1:datasetsnum]}
  
  if (any(sapply(1:datasetsnum, function(x) (lag[x] != 0 && w[x] != 1)))) {if (verbose) message("Phenotypic lag cannot be used together with mutant fitness."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  
  if (any(is.null(death)) || any(is.na(death))) {death <- rep(0, datasetsnum)}
  death <- unlist(death)
  if (length(death) < datasetsnum) {if (verbose) message("The vector of relative death rates is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(death) > datasetsnum) {if (verbose) message(paste("The vector of relative death rates is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); death <- death[1:datasetsnum]}
  
  if (any(is.null(poisson)) || any(is.na(poisson))) {poisson <- rep(0, datasetsnum)}
  poisson <- unlist(poisson)
  if (length(poisson) < datasetsnum) {if (verbose) message("The vector of the sizes of residual mutations is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(poisson) > datasetsnum) {if (verbose) message(paste("The vector of the sizes of residual mutations is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); poisson <- poisson[1:datasetsnum]}
  
  if (any(is.null(phi)) || any(is.na(phi))) {phi <- rep(0, datasetsnum)}
  phi <- unlist(phi)
  if (length(phi) < datasetsnum) {if (verbose) message("The vector of relative inoculum sizes is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(phi) > datasetsnum) {if (verbose) message(paste("The vector of relative inoculum sizes is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); phi <- phi[1:datasetsnum]}
  
  if (any(is.null(Nt)) || any(is.na(Nt))) {Nt <- rep(1, datasetsnum)}
  Nt <- unlist(Nt)
  if (length(Nt) < datasetsnum) {if (verbose) message("The vector of average numbers of cells in cultures is shorter than the length of data."); if (combo_rate) return(c(NA,NA)) else return(NA)}
  else if (length(Nt) > datasetsnum) {if (verbose) message(paste("The vector of average numbers of cells in cultures is longer than the length of data; only first ", datasetsnum, " values will be used.", sep = "")); Nt <- Nt[1:datasetsnum]}
  Nt <- Nt*(1-phi)
  
  # Estimating m
  if (any(!is.finite(M)) || length(M) != datasetsnum) {
    M <- sapply(1:datasetsnum, function(x) {if (verbose) message(paste("Estimating mutation rate for dataset ", x, "...", sep = "")); mle.mutations(data = data[[x]], e = e[x], w=w[x], lag=lag[x], poisson=poisson[x], death=death[x], phi=phi[x], cv = cv[x], confint = FALSE, verbose=verbose)})
  }
  if (any(!is.finite(M))) {if (verbose) message("Couldn't estimate mutation rates"); if (combo_rate) return(c(NA,NA)) else return(NA)}
  if (verbose) message(paste("Estimated numbers of mutations are:", paste(M, collapse = ", ")))
  if (verbose) message(paste("Estimated mutation rates are:", paste(M/Nt, collapse = ", ")))
  ...
  e <- c(e[max], e[-max])
  w <- c(w[max], w[-max])
  lag <- c(lag[max], lag[-max])
  poisson <- c(poisson[max], poisson[-max])
  death <- c(death[max], death[-max])
  phi <- c(phi[max], phi[-max])
  cv <- c(cv[max], cv[-max])
  Nt <- c(Nt[max], Nt[-max])
  M <- c(M[max], M[-max])
  n <- sapply(data, max)+1
  k <- sapply(cv, function(x) {ifelse(x>0, 1/x/x, 0)})
  R <- Nt/Nt[1]
  seq <- mapply(FUN = function(e,w,death,lag,phi,n) {aux.seq(e,w,death,lag,phi,n-1)}, e = e, w = w, death = death, lag = lag, phi = phi, n = n, SIMPLIFY = FALSE)
  
  # Combined mutation rate
  m.est <- combo_optim_m_new(current_m = (min(M)+max(M))/2, lower_m = min(M),
                             upper_m = max(M), R = R, seqs = seq, len = n,
                             datasetsnum = datasetsnum, datas = data, k, poisson = poisson, verbose = verbose)
  if (m.est[1] < 0) {if (verbose) message("m_combined: couldn't estimate m"); if (combo_rate) return(c(NA,NA)) else return(NA)}
  ...
  if (MC != 0) {
    ...
    l0 <- 0
    for (i in 1:datasetsnum) {
      l <- logprob(m=1e-200, len=n[i], data=data[[i]], seq=seq[[i]], k=k[i], poisson=poisson[i])
      if (!is.finite(l)) {l <- logprob_boost(xm = 1e-200, len=n[i], data=data[[i]], seq=seq[[i]], xk=k[i], xpoisson=poisson[i])}
      if(!is.finite(l)) {if (verbose) message("loglikelihood: non-numeric value encountered"); if (combo_rate) return(c(NA,NA)) else return(NA)}
      l0 <- sum(l0,l)
    }
  }
  ...
  l1 <- 0
  for (i in 1:datasetsnum) {
    l <- logprob(m=M[i], len=n[i], data=data[[i]], seq=seq[[i]], k=k[i], poisson=poisson[i])
    if (!is.finite(l)) {l <- logprob_boost(xm = 1e-200, len=n[i], data=data[[i]], seq=seq[[i]], xk=k[i], xpoisson=poisson[i])}
    if(!is.finite(l)) {if (verbose) message("loglikelihood: non-numeric value encountered"); if (combo_rate) return(c(NA,NA)) else return(NA)}
    l1 <- sum(l1,l)
  }
  ...
  chi <- -2*(l0-l1)
  pval <- pchisq(chi,datasetsnum-1,lower.tail = F)
  
  if (combo_rate) return(c(pval, MC/Nt[1])) else return(pval)
}
