...
loadDatasetWrapper <- function(id, rv, SettingsPlatingID) {
  callModule(countsPlatingLoadDataset,
             ...
             CountsStrain7FoldUserInput = reactive(rv$CountsStrain7FoldUserInput()),
             CountsStrain8FoldUserInput = reactive(rv$CountsStrain8FoldUserInput()),
...
comboValidator <- function(DataPlating, DataSelective, DataNonselective) {
  items <- c("VolumeTotal", "Fitness", "Lag", "Residual", "Death", "Inoculum",
             ...
             "VolumeNonselective", "DilutionNonselective", "MeanCells", "CV", "ConfidenceLevel")
  ...
  DataPlating2 <- data.frame(matrix(data = numeric(), nrow = items.length, ncol = length(DataPlating[-1]), dimnames = list(items, colnames(DataPlating[-1]))))
  ...
  rownames(ParameterExists)[items.length+1] <- "CountsSelective"
  rownames(ParameterExists)[items.length+2] <- "CountsNonselective"
  ...
  if (length(DataPlating)>=2) {
    ...
    for (i in 2:length(DataPlating)) {
      ...
      # Checking if confidence interval limits should be evaluated
      if (ParameterExists["ConfidenceLevel",i-1]==TRUE) {
        if (is.na(as.numeric(DataPlating[parameterNumbers$ConfidenceLevel,i])) == TRUE) {
          DataPlating[parameterNumbers$ConfidenceLevel,i] <- paste(DataPlating[parameterNumbers$ConfidenceLevel,i], warningTooltip("Non-numeric characters detected."), sep = " ")
          CellNonnumeric <- CellNonnumeric+1
          colsWithErrors <- c(colsWithErrors, i-1)
        } else if (as.numeric(DataPlating[parameterNumbers$ConfidenceLevel,i]) <= 0 | as.numeric(DataPlating[parameterNumbers$ConfidenceLevel,i]) >= 1) {
          DataPlating[parameterNumbers$ConfidenceLevel,i] <- paste(DataPlating[parameterNumbers$ConfidenceLevel,i], warningTooltip("Confidence level must bigger than 0 (&gt;0) and smaller than 1 (&lt;1)."), sep = " ")
          CellNonpositive <- CellNonpositive+1
          colsWithErrors <- c(colsWithErrors, i-1)
        } else {
          DataPlating2["ConfidenceLevel",i-1] <- as.numeric(DataPlating[parameterNumbers$ConfidenceLevel,i])
        }
      } else {
        DataPlating2["ConfidenceLevel",i-1] <- 0.95
      }
    }
